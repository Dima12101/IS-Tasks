...
class lin_reg_model:
    def fit(self, X, Y, start_b=None, algorithm="RMSProp"):
        if start_b is None:
        ...
        if algorithm == "RMSProp":
            b, err = RMSprop(X, Y, start_b, scale_coef=0.8, reg_par=1, num_iter=50)
        elif algorithm == "Adam":
            b, err = Adam(X, Y, start_b, scale_coef_M=Y.mean(), scale_coef_G=0.1, reg_par=0.8)
        elif algorithm == "Adagrad":
            b, err = Adagrad(X, Y, start_b, reg_par=1, num_iter=50)
        elif SGD:
            b, err = SGD(X, Y, start_b, reg_par=1, num_iter=50, logging=False)

        self.b = b

    def predict(self, X):
        return np.dot(X, self.b[:-1]) + self.b[-1]
...
def calculate_error_with_regul(X, Y, b, reg_par):
    regul_error = reg_par * (b[:-1] ** 2 + abs(b[:-1]))
    pred = (X * b[:-1] + b[-1]).sum(axis=1)
    return ((Y - pred) ** 2).sum() + regul_error.sum()
...
def SGD(X, Y, b, reg_par = 0, num_iter=50, logging=False):
    coef_grad = 1
    coef_prev_grad = 0.2
...
def Adagrad(X, Y, b, reg_par = 0, num_iter = 50, logging=False):
    coef_grad = 0.1
...
def RMSprop(X, Y, b, scale_coef=0.5, reg_par = 0, num_iter = 50, logging=False):
    coef_grad = 0.1
    G_vector = np.zeros(len(b)) + 0.01
    G_vector_sqr = G_vector ** 2
...
def Adam(X, Y, b, scale_coef_M = 0.5, scale_coef_G = 0.5, reg_par = 0, num_iter = 50, logging=False):
